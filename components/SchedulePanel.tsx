...
const styles = {
  ...
  canceled: "border-canceled/30 bg-canceled/10 text-slate-800",
  recovering: "border-amber/40 bg-amber/10 text-slate-800",
  recovered: "border-recovered/40 bg-recovered/10 text-slate-800",
  unfilled: "border-slate-200 bg-white text-slate-600"
};
...
export function SchedulePanel() {
  ...
  return <Panel title="Tuesday Schedule" eyebrow="Live clinic day" className="lg:row-span-2">
    <div className="mb-3 flex items-center gap-2 text-sm text-slate-600"><CalendarDays size={16}/> Dr. Rivera · Room 2</div>
    <div className="space-y-2">
      <AnimatePresence initial={false}>
        {appointments.map((appointment) => <motion.div layout key={appointment.id} initial={{opacity:0,y:4}} animate={{opacity:1,y:0}} className={`rounded-2xl border p-3 transition ${styles[appointment.status]}`}>
          ...
          <p className="text-xs text-slate-600">{appointment.appointmentType.replaceAll("-", " ")} · {appointment.duration} min</p>
        </motion.div>)}
      </AnimatePresence>
    </div>
  </Panel>;
