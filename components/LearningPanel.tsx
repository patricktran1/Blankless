...
export function LearningPanel() {
  ...
  return <Panel title="Learning Policy" eyebrow="Versioned heuristic">
    ...
    <div className="space-y-2">{entries.map(([key,value]) => { const old = previousPolicy?.weights[key as keyof typeof policy.weights]; const delta = old === undefined ? 0 : value-old; return <div key={key}><div className="mb-1 flex justify-between text-[11px]"><span>{key.replace(/([A-Z])/g," $1")}</span><span className="font-bold">{value}% {delta !== 0 && <b className={delta > 0 ? "text-recovered" : "text-canceled"}>({delta>0?"+":""}{delta})</b>}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-100"><motion.div initial={{width:0}} animate={{width:`${value}%`}} className="h-full rounded-full bg-teal"/></div></div>})}</div>
    {comparison && <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="mt-4 rounded-2xl border border-recovered/30 bg-recovered/10 p-3 text-sm font-semibold text-recovered">{comparison}</motion.div>}
  </Panel>;
}
