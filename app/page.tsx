...
export default function Home() {
  ...
  return <main className="mx-auto min-h-screen max-w-[1500px] px-4 py-6 md:px-8">
    ...
    <div className="sticky bottom-4 z-20 mx-auto mt-6 flex w-fit flex-wrap items-center justify-center gap-2 rounded-2xl border border-black/10 bg-white/90 p-2 shadow-2xl backdrop-blur">
      ...
      <button disabled={running || policy.id !== "policy-v2" || metrics.recovered !== 1} onClick={() => playScenario(2)} className="flex items-center gap-2 rounded-xl bg-teal px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-35"><Play size={15}/> Play Scenario 2</button>
